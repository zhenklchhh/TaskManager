...
func (r PostgresTaskRepository) GetTaskStatsForCompany(ctx context.Context, companyID uuid.UUID) (*domain.TaskStats, error) {
	stats := &domain.TaskStats{
		TasksByType:     make(map[string]int),
		TasksByPriority: make(map[int]int),
	}
	...
	rows, err := r.pool.Query(ctx, `SELECT type, COUNT(*) FROM tasks WHERE company_id = $1 GROUP BY type`, companyID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	rows2, err := r.pool.Query(ctx, `SELECT priority, COUNT(*) FROM tasks WHERE company_id = $1 GROUP BY priority`, companyID)
	if err != nil {
		return nil, err
	}
	defer rows2.Close()
